...
正如我们所讨论的，图结构G编码一组条件独立性假设$I_l(G)$。 仅知道分布P在G上分解，我们可以得出结论，P满足$I_l(G)$。 一个直接的问题是，我们是否可以直接从G中“读出”其他独立性。也就是说，对于分解G的每个分布P是否存在其他独立性？

### 3.3.1 D-separation

本节的目的是了解何时可以保证与BN结构G关联的分布中具有独立性$(X \perp Y \mid Z)$。要了解何时保证拥有某属性，考虑其相反情况是有帮助的： “我们能想象一个不发生这种情况的情况吗？”因此，我们将讨论重点放在分析在给定Z的情况下X可能会影响Y的情况。如果我们构造一个发生这种影响的示例，则相反的属性$(X\perp Y \mid Z)$不能满足所有分解为G的分布，因此独立性$(X\perp Y \mid Z)$不能从$I_l(G)$得到。

因此，我们从直观的案例分析开始：在这里，我们尝试了解在变量Z的证据存在的情况下，关于变量X的观察何时可能改变我们对Y的信念。尽管这种分析将是完全直观的，但我们稍后将证明我们的结论实际上是可以被证明是正确的。

**Direct connection**

**Indirect connection**

**Indirect causal effect (figure 3.5a).**

**Indirect evidential effect (figure 3.5b).**

**Common cause (figure 3.5c).**

**Common effect (figure 3.5d).**
